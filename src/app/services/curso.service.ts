...
export class CursoService {
  ...
  saveCurso(curso: curso): Observable<any> {
    return this.http.post<any>("http://localhost:8082/cursos/save", curso);
  }

  createUsuario(usuario: any): Observable<any> {
    return this.http.post<any>("http://localhost:8080/micro/save", usuario);
  }

  getAllCursos(): Observable<any> {
    return this.http.get<any>("http://localhost:8080/micro/getCursoall");
  }
  ...
  asignarCurso(userId: string, courseId: string): Observable<any> {
    const url = "http://localhost:8080/micro/inscripciones/save";
